...
const Header = () => {
  ...
  const activeLink = 'text-red-600';
  const normalLink = 'relative after:bg-red-600 after:h-[0.1em] after:absolute after:-bottom-1 after:left-0 after:hover:w-[100%] after:hover:animate-[hoverWidth_0.5s_linear]';
  ...
  return(
    <div className="flex justify-between items-center shadow-lg fixed top-0 w-[100%] bg-white z-30 p-6">
        ...
        <ul className='flex justify-evenly p-4 font-medium laptop:text-lg text-base items-center cursor-pointer
         ...
         { /*<li>Online Status : {onlineStatus ? 
          ...
          <li className='relative'>
            ...
            <span className='absolute laptop:bottom-[1.1rem] bg-red-500 px-[0.3rem] rounded-[50%] text-white font-bold 
      ...
      <div className={`relative tablet:hidden right-[2rem] ${toggleMenu ? '-z-10' : ''}`}>
        ...
        <span className='absolute right-0 bottom-[1.1rem] bg-red-500 px-[0.3rem] rounded-[50%] text-white font-bold 
          text-xs'>
          {cartItems}
        </span>
      </div>
